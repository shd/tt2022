...
дизъюнкт, $P(\theta_i)$ --- предикатный символ $P$ с набором аргументов $\theta_i$,
и $\mathcal{S}$ --- наиболее общий унификатор $P(\theta_1)$ и $P(\theta_2)$).
...
\item Напишите логическую формулу, утверждающую, что двухбитное число $z$ (также заданное переменными $Z_i$) является результатом деления 
числа $x$ на число $y$. 
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
\item (5 баллов). Покажите, что если $a,b \in \mathbb{N}_0$ и $a \le b$, то $2a \le a+b \le 2b$.
\item (5 баллов). Покажите, что $x^m < x^n$, если $x > 1$ и $m < n$ ($x,m,n \in \mathbb{N}_0$).
\item (5 баллов). Покажите, что если $a,b,c \in \{0,1\}$, то $a=b$, $b=c$ или $a=c$.
\item (5 баллов). Покажите, что неверно, что cуществует такое натуральное $k$, что все числа, большие $k$, --- составные.
В чём отличие этого утверждения от прямого утверждения (найдётся сколь угодно большое простое число) --- как доказать
прямое утверждение (приведите идею доказательства в комментарии)?
\item (5 баллов). Покажите, что $n! \le n^n$ при $n > 1$.
\item (5 баллов). Покажите, что произведение непустого списка натуральных чисел равно 0 тогда и только тогда, когда
среди этих чисел есть хотя бы один 0.
\item (5 баллов). Покажите, что $a \le b$ тогда и только тогда, когда $a \dotminus b = 0$ ($a,b \in \mathbb{N}_0$).
\item (7 баллов). Покажите признак деления на 3: если сумма цифр в списке $(a_0,a_1,\dots,a_{n-1})$ делится на 3, то число, составленное из
этих цифр ($a_0 + a_1 \cdot 10 + \dots + a_{n-1}\cdot 10^{n-1}$) также делится на 3.
\item (8 баллов). Покажите, что на множестве из четырёх значений можно задать поле (\verb!Field!) --- напомним, что
в случае $\mathbb{F}_4$ можно получить поле из значений $\{0,1,x,x+1\}$, где $x$ --- корень многочлена
$x^2 + x + 1$, неприводимого над полем $\mathbb{F}_2$.
...
\item (10 баллов). Определите на LiquidHaskell (или F*) тип <<перестановка>>, функцию композиции перестановок -- и покажите,
что тождественная перестановка --- единица в смысле композиции перестановок.
\item (15 баллов как независимая задача; 7 баллов, если вместе с предыдущей). Определите на LiquidHaskell (F*) функцию получения обратной 
перестановки --- тип которой утверждает, что обратная перестановка в композиции с прямой есть тождественная перестановка.
\end{enumerate}
