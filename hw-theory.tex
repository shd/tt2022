...
\verb!\truncate! (убедитесь, что аналог \verb!integer_eq! скомпилируется).
...
\item По \verb!not-equals : (x > y || x < y)! (обратите внимание, здесь применяется усечённое <<или>>) получите \verb!x /= y!.
...
по отношению $a\cdot d = b \cdot c$ ($a,c \in \mathbb{N}_0$, $b,d \in \mathbb{N}$).
